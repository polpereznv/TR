...
def crop_face_from_webcam(grey):
    ...
    if len(faces) == 0:
        ...
        print(f"{len(faces)} face(s) detected.")
        x, y, w, h = faces[0]
    foto = Image.open(grey)
    crop_area = (x, y, x + w, y + h)
    imatge_retallada = foto.crop(crop_area)
    cropped_face = np.array(imatge_retallada)
    return cropped_face
...
"""
def from_foto_to_pixels(foto,findRectangle=False,PrintFoto=False):
    face_classifier = cv2.CascadeClassifier(cv2.data.haarcascades + "haarcascade_frontalface_default.xml")
    facemark = cv2.face.createFacemarkLBF()
    facemark.loadModel("/home/polperez/Desktop/lbfmodel.yaml")
    grey_scale = cv2.cvtColor(foto, cv2.COLOR_BGR2GRAY)
    faces = face_classifier.detectMultiScale(grey_scale, scaleFactor = 1.1, minNeighbors = 5)
    for (x, y, w, h) in faces:
